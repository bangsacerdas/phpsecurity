...
// refinement: HTML5 elements "email" and "number"
// why use: additional browser-side validation
// caution: (1) can be easily copied and modified by attackers,
//			    so do not forget filtering and validation in your PHP code!!!
//			(2) support is not consistent across all browsers
//				i.e. type="number" not supported by Firefox
...
$default['email']	= 'email@company.com';
...
// $email = defaults = "email@company.com"
$email 	= (isset($_POST['email'])) ? strip_tags($_POST['email']) : $default['email'];
...
$maxLength['name'] 	= 128;
$maxLength['image'] = 128;
$error 				= array('id' => '', 'name' => '', 'image' => '', 'email' => '', 'hash' => '');
...
// $email = use filter_var()
if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
	$valid++;
} else {
	$error['email'] = 'Please re-enter your email address!';
}
...
<body>
<h1>protect_against_xss_html5_1.php</h1>

Same as <i>protect_against_xss_forms_1.php</i> adding HTML5 elements

<hr />
<!-- There are a total of 5 fields to be validated -->
Data is saved: <b><?php echo ($valid == 5) ? 'YES' : 'NO'; ?></b>
<hr />
<form method="post" action="protect_against_xss_html5_1.php">
<table>
<tr>
	<th>ID</th>
	<!-- NOTE: maxlength -->
	<td><input type="number" min="1" max="<?php echo $maxId; ?>" name="id" size=8 maxlength=8 value="<?php echo $id; ?>" /></td>
	<td>Current Value: <?php echo $id; ?></td>
	<td><b style="color:red;"><?php echo $error['id']; ?></b></td>
</tr>
<tr>
	<th>Name</th>
	<!-- NOTE: using HTML5 "placeholder" attribute -->
	<td><input type="text" name="name" maxlength=128 placeholder="<?php echo htmlspecialchars($name); ?>" /></td>
...
<tr>
	<th>Image</th>
	<td><input type="text" name="image" maxlength=128 placeholder="<?php echo htmlspecialchars($image); ?>" /></td>
	<td>Current Value: <img src="<?php echo htmlspecialchars($image); ?>" /></td>
	<td><b style="color:red;"><?php echo $error['image']; ?></b></td>
</tr>
<tr>
	<th>Email</th>
	<td><input type="email" name="email" maxlength=128 placeholder="<?php echo htmlspecialchars($email); ?>" /></td>
	<td>Current Value: <img src="<?php echo htmlspecialchars($email); ?>" /></td>
	<td><b style="color:red;"><?php echo $error['email']; ?></b></td>
</tr>
</table>
